...
| `cli.py` | 参数、读取、解析、退出码、原子 DOCX 保存、覆盖检查、可选 PDF 导出。 | JSON 字段规则与 Word 排版细节。 |
...
`cli.main()` 的稳定顺序是：读取输入 → 解析 locale/内容 → （`--pdf` 时预检 PDF 输出目标）→ 渲染 → 检查并原子保存 DOCX → 可选导出 PDF。DOCX 输出目标由保存步骤检查，因此已有 DOCX 不会被覆盖，但会在完成渲染后才报告冲突。

- 未指定 `-o` 时，输出写入项目根目录的 `output/`，并与输入同名。
- 指定 `-o` 时，父目录必须已经存在；默认输出目录可由 CLI 创建。
